rule beta_group_sign:
    ...
    output:
        expand(qiime2_dir("group_significances", "{{bgs_method}}", "{meta_col}", "jaccard_group_significance.qzv"), meta_col = META_COLS),
        expand(qiime2_dir("group_significances", "{{bgs_method}}", "{meta_col}", "bray_curtis_group_significance.qzv"), meta_col = META_COLS),
        expand(qiime2_dir("group_significances", "{{bgs_method}}", "{meta_col}", "aitchison_group_significance.qzv"), meta_col = META_COLS)
    conda:
        conda_qiime2
    params:
